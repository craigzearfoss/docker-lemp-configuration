...
create_docker_containers() {
  ...
  # Make changes to php-7-4-fpm file.
  if [[ $project_framework == "Yii" ]]; then
    sed -i "s/root \/var\/www\/public/root \/var\/www\/${project_name}\/web/g" ${nginx_conf_file}
  else
    sed -i "s/root \/var\/www\/public/root \/var\/www\/${project_name}\/public/g" ${nginx_conf_file}
  fi
  sed -i "s/error_log  \/var\/log\/nginx\/error.log;/error_log  \/var\/log\/nginx\/${project_name}_error.log;/g" ${nginx_conf_file}
  sed -i "s/access_log \/var\/log\/nginx\/access.log;/access_log \/var\/log\/nginx\/${project_name}_access.log;/g" ${nginx_conf_file}

  export DB_DATABASE="${mysql_database}"
  export DB_ROOT_PASSWORD="${mysql_root_password}"
  export DB_PASSWORD="${mysql_user_name}"
  export DB_USERNAME="${mysql_user_password}"
...
unimplemented_frameworks=("CakePHP WordPress Zend")
...
if [ -d $project_dir ]; then
  printf "\nThe directory ${project_name} already exists.\n"
  exit
fi
...
printf "You should now be able to the to http://localhost:${port} in your browser.\n"
exit
